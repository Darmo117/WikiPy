...
from .. import page_context, settings, api, forms
...
class LoginPageContext(page_context.PageContext):
    ...
    def __init__(self, context: page_context.PageContext, /, login_notice: str, login_warning: str = None,
                 form: forms.LogInForm = None, global_errors: typ.List[str] = None):
...
class LoginPage(SpecialPage):
    ...
    def _get_data_impl(self, sub_title, base_context, request, **kwargs):
        ...
        login_notice = dj_safe.mark_safe(api.render_wikicode(
            api.get_message('LoginDisclaimer')[0],
            base_context
        ))
    ...
    def __get_form(base_context: page_context.PageContext, request: dj_wsgi.WSGIRequest, login_notice: str) \
        ...
        if user.is_logged_in:
            login_warning = dj_safe.mark_safe(api.render_wikicode(
                api.get_message('AlreadyLoggedIn')[0],
                base_context
            ))
        else:
            login_warning = None
        ...
        return LoginPageContext(base_context, login_notice=login_notice, login_warning=login_warning,
                                form=forms.LogInForm(initial=request.GET))
